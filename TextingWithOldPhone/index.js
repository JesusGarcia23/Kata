...
const sendMessage = message => {
    ...
    function typeText(value, letter, index, next) {
      const actualIsCase = (letter === letter.toUpperCase());
       let text = "";
       if((letter !== " ") && (".,?!'-+=".indexOf(letter) < 0)){
        text = checkCase(letter)
       }
    ...
    function checkNextValue(actualValue, obj, nextValue) {
      if(keyboard[nextValue] !== undefined){
        return keyboard[nextValue].toString().indexOf(actualValue.toLowerCase()) >= 0 ? 
        true : false
      }else if(obj.toString().indexOf(nextValue.toLowerCase()) >= 0){
      return (actualValue === actualValue.toUpperCase()) === (nextValue === nextValue.toUpperCase());
      }else{
        return false;
      }
    }
    ...
    function checkCase(value) {
      const letterCase = (value === value.toUpperCase() ) ? "upper" : "lower";
      if(letterCase !== actualCase) {
        actualCase = letterCase
        return "#";
      }else{
        return "";
      }
    ...
    for(let i = 0; i < message.length; i++) {
      for (let key of Object.keys(keyboard)) {
       ...
       else if(keyboard[key].toString().indexOf(message[i].toLowerCase()) >= 0) {
          let nextNumberIndex = i < message.length - 1 ? checkNextValue(message[i], keyboard[key], message[i+1]) : null;

          let actualNumberIndex = keyboard[key].toString().indexOf(message[i].toLowerCase()) + 1;

          result += typeText(key, message[i], actualNumberIndex, nextNumberIndex);
          break;
        }
  }
    }
